...
class ParquetFile(object):
    ...
    def read_row_group(self, rg, columns, categories, infile=None,
        ...
        core.read_row_group(
                ...
                self.selfmade, index=index, assign=assign, mr_times=mr_times)
        if ret:
            return df
    ...
    def to_pandas(self, columns=None, categories=None, filters=[],
        ...
        Pandas data-frame
        """
        check_column_names(self.columns, columns, categories, mr_times)
        rgs = self.filter_row_groups(filters)
        ...
        if self.file_scheme == 'simple':
            ...
            for rg in rgs:
                parts = {name: (v if name.endswith('-catdef')
                         ...
                         for (name, v) in views.items()}
                self.read_row_group_file(rg, columns, categories, index,
                                         assign=parts, mr_times=mr_times)
                start += rg.num_rows
        return df

    def pre_allocate(self, size, columns, categories, index, mr_times=[]):
        return _pre_allocate(size, columns, categories, index, self.cats,
                             self.dtypes, mr_times=mr_times)
...
def _pre_allocate(size, columns, categories, index, cs, dt, mr_times=[]):
